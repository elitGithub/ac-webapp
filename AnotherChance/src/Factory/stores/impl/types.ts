...
export type Updater<T = any> = (value: T) => T;

export type UpdateAction<T extends object> = (value: T) => void;

export interface CallableState<T = any> {
	(): T;
...
export interface MutableCallableState<T extends object> extends CallableState<T> {
	mutable: UpdateAction<T>;
}

export interface ReadonlyCallableState<T = any> {
	(): T;
	subscribe: Subscriber<T>;
}
...
export type Invalidator<T = any> = (value?: T) => void;

/** Pair of subscriber and invalidator. */
export type SubscribeInvalidateTuple<T = any> = [Subscriber<T>, Invalidator<T>];
